        ...
        <div class="container">
            <nav class="navbar navbar-expand-lg navbar-light px-0 py-3">
                <a href="{{ route('home') }}" aria-label="{{ config('settings.title') }}" class="navbar-brand p-0">
                    <div class="logo">
                        <img src="{{ url('/') }}/uploads/brand/{{ (request()->cookie('dark_mode') == 1 ? config('settings.logo_dark') : config('settings.logo')) }}" data-theme-dark="{{ url('/') }}/uploads/brand/{{ config('settings.logo_dark') }}" data-theme-light="{{ url('/') }}/uploads/brand/{{ config('settings.logo') }}" data-theme-target="src">
                    </div>
                </a>
    ...
    <nav class="slide-menu shadow bg-base-0 navbar navbar-light p-0 d-flex flex-column z-1030 d-print-none" id="slide-menu">
        <div class="sidebar-section flex-grow-1 d-flex flex-column w-100">
            ...
            <div class="d-flex align-items-center">
                ...
                @if(Auth::user()->role == 1)
                    @if (request()->is('admin/*'))
                        <a class="px-4 py-2 text-decoration-none text-secondary" href="{{ route('dashboard') }}" data-enable="tooltip" title="{{ __('User') }}" role="button"><span class="d-flex align-items-center">@include('icons.account-circle', ['class' => 'width-4 height-4 fill-current'])</span></a>
                    @else
                        <a class="px-4 py-2 text-decoration-none text-secondary" href="{{ route('admin.dashboard') }}" data-enable="tooltip" title="{{ __('Admin') }}" role="button"><span class="d-flex align-items-center">@include('icons.supervised-user-circle', ['class' => 'width-4 height-4 fill-current'])</span></a>
                    @endif
                @endif
            </div>
            ...
            <div class="sidebar sidebar-footer">
                <div class="py-3 {{ (__('lang_dir') == 'rtl' ? 'pr-4 pl-0' : 'pl-4 pr-0') }} d-flex align-items-center" aria-expanded="true">
                    ...
                    <a class="py-2 px-4 d-flex flex-shrink-0 align-items-center text-secondary" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" data-enable="tooltip" title="{{ __('Logout') }}">@include('icons.exit-to-app', ['class' => 'fill-current width-4 height-4'])</a>
